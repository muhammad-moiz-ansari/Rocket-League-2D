...
class Ball:
    def __init__(self):
        ...
        self.texture_key = 'ball'
        self.fallback_color = ORANGE
        self.reset()
    ...
    def update(self):
        # UPDATED: Using self.friction which can be set per game mode
        self.vx *= self.friction
        self.vy *= self.friction
    ...
    def draw(self, surf):
        # Try to use the texture key specific to game mode
        texture = assets_loader.GRAPHICS.get(self.texture_key)
        if texture:
            ...
            # Fallback to color
            pygame.draw.circle(surf, self.fallback_color, (int(self.x), int(self.y)), self.radius)
